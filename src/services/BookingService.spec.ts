...
it('findRoomsOccupation', async () => {
  vi.spyOn(agendaService, 'findEventsOfDay').mockImplementation(() =>
    Promise.resolve(EVENTS)
  );
  const bookingService = new BookingService(agendaService);
  // 9h30 -> Minuit
  const occupations = await bookingService.findRoomsOccupation(
    '2025-12-13',
    1765620000000,
    1765679940000,
    []
...
it('availableTablesByRooms', async () => {
  ...
  const availableByRooms = await bookingService.availableTablesByRooms(
    '2025-12-13',
    1765620000000,
    1765679940000,
    []
